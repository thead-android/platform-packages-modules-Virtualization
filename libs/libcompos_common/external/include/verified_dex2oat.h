 ...
 */
typedef struct AVerifiedDex2Oat_CompilationContext {
    void* _Nonnull ctx;
} AVerifiedDex2Oat_CompilationContext;

/**
 * An opaque struct containing the user provided and owned opaque data.
 */
typedef struct AVerifiedDex2Oat_SuccessCallbackContext {
    void* _Nonnull ctx;
} AVerifiedDex2Oat_SuccessCallbackContext;

/**
 * An opaque struct containing information about a successful compilation.
 */
typedef struct AVerifiedDex2Oat_SuccessResultContext {
    const void* _Nonnull ctx;
} AVerifiedDex2Oat_SuccessResultContext;

/**
 * An opaque struct containing the user provided and owned opaque data.
 */
typedef struct AVerifiedDex2Oat_FailureCallbackContext {
    void* _Nonnull ctx;
} AVerifiedDex2Oat_FailureCallbackContext;

/**
 * An opaque struct containing information about a failed compilation.
 */
typedef struct AVerifiedDex2Oat_FailureResultContext {
    const void* _Nonnull ctx;
} AVerifiedDex2Oat_FailureResultContext;
...
typedef enum AVerifiedDex2Oat_Status : int32_t {
     ...
     * Introduced in API Level 37.
     */
    AVERIFIED_DEX2OAT_CTX_NOT_STARTED = -20,
...
typedef void (*AVerifiedDex2Oat_OnSuccessCallback)(
        AVerifiedDex2Oat_SuccessCallbackContext success_cb_ctx,
        AVerifiedDex2Oat_SuccessResultContext result_ctx);
...
AVerifiedDex2Oat_Status AVerifiedDex2Oat_CompilationStats_getWallClockTimeMs(
        const AVerifiedDex2Oat_SuccessResultContext success_result_ctx,
...
typedef void (*AVerifiedDex2Oat_OnFailureCallback)(
        AVerifiedDex2Oat_FailureCallbackContext failure_cb_ctx,
        AVerifiedDex2Oat_FailureResultContext results_ctx);
...
AVerifiedDex2Oat_Status AVerifiedDex2Oat_FailureInfo_getFailureCode(
        AVerifiedDex2Oat_FailureResultContext failure_result_ctx,
...
AVerifiedDex2Oat_Status AVerifiedDex2Oat_FailureInfo_getMessage(
        AVerifiedDex2Oat_FailureResultContext failure_result_ctx, const char** _Nonnull message)
        __INTRODUCED_IN(37);

/* A pointer to the pointer that `AVerifiedDex2Oat_createCompilationContext` will modify
 * so that it instead points to the newly created compilationContext.
 * Introduced in API Level 37.
 */
typedef AVerifiedDex2Oat_CompilationContext* CompilationContextPtr;
 ...
 * @param on_success_cb After a compilation is started using `AVerifiedDex2Oat_start` this function
 * will be called if the compilation is successful.
 ...
 * @param on_failure_cb After a compilation is started using `AVerifiedDex2Oat_start` this function
 * will be called if the compilation fails.
 ...
 *
 * @return `AVERIFIED_DEX2OAT_SUCCESS` on success or
...
AVerifiedDex2Oat_Status AVerifiedDex2Oat_createCompilationContext(
        CompilationContextPtr _Nonnull out_ctx,
 ...
 *
 * This function takes a pointer to a context and frees it.
 *
 * This can safely be called on a context that has never had AVerifiedDex2Oat_start called on
 * it.
 *
 ...
 *
 */
void AVerifiedDex2Oat_destroyCompilationContext(AVerifiedDex2Oat_CompilationContext in_ctx)
        __INTRODUCED_IN(37);
__END_DECLS