...
use fs_crypt_bindgen::{
    fscrypt_add_key_arg, fscrypt_key_specifier, fscrypt_key_specifier__bindgen_ty_1,
    fscrypt_policy_v2, fscrypt_remove_key_arg, FSCRYPT_KEY_IDENTIFIER_SIZE,
...
// Rust wrapper of FS_IOC_ADD_ENCRYPTION_KEY
fn fs_ioc_add_encryption_key(
...
// Rust wrapper of FS_IOC_SET_ENCRYPTION_POLICY
fn fs_ioc_set_encryption_policy(
...
// Removes an encryption key from the filesystem specified by fd
// Rust wrapper of FS_IOC_REMOVE_ENCRYPTION_KEY
fn fs_ioc_remove_encryption_key(
    key_identifier: &[u8; FSCRYPT_KEY_IDENTIFIER_SIZE as usize],
    fd: &OwnedFd,
) -> std::io::Result<()> {
    const FS_IOC_REMOVE_ENCRYPTION_KEY: libc::c_int = 0xc0406618u32 as i32;

    let remove_key = fscrypt_remove_key_arg {
        key_spec: fscrypt_key_specifier {
            type_: FSCRYPT_KEY_SPEC_TYPE_IDENTIFIER,
            __reserved: 0,
            u: fscrypt_key_specifier__bindgen_ty_1 { identifier: *key_identifier },
        },
    ...
    // SAFETY: Using correct struct for ioctl
    unsafe {
        match libc::ioctl(fd.as_raw_fd(), FS_IOC_REMOVE_ENCRYPTION_KEY, &remove_key) {
            0 => Ok(()),
            _ => Err(std::io::Error::last_os_error()),
        }
    }
...
pub fn set_encryption_key(file_path: &Path, key: &[u8]) -> std::io::Result<[u8; 16]> {
    ...
    let key_identifier = fs_ioc_add_encryption_key(&secret_key, &file)?;
    fs_ioc_set_encryption_policy(&key_identifier, &file)?;
    info!("Successfully enabled fs-crypt for: {}", file_path.display());
    Ok(key_identifier)
}

// Takes a key and removes it from the filesystem containing file_path
pub fn remove_encryption_key(file_path: &Path, key_identifier: &[u8; 16]) -> std::io::Result<()> {
    let file = open(file_path, OFlags::RDONLY | OFlags::DIRECTORY, Mode::empty())?;
    fs_ioc_remove_encryption_key(key_identifier, &file)?;
    info!("Successfully removed encryption key from: {}", file_path.display());
    Ok(())
}
