...
pub fn get_service_connection_info(name: &str, cid: u32) -> binder::Result<Vsock> {
    ...
    let stats_service_instance =
        <aidl::BpStats as aidl::IStats>::get_descriptor().to_owned() + "/default";
    ...
    let local_svc: Strong<dyn aidl::IStats> = binder::check_interface(name)
    ...
    let service = aidl::BnStats::new_binder(
        StatsDelegator { delegate: local_svc },
...
struct StatsDelegator {
    delegate: Strong<dyn aidl::IStats>,
}

impl Interface for StatsDelegator {}

impl aidl::IStats for StatsDelegator {
    fn reportVendorAtom(&self, atom: &aidl::VendorAtom) -> binder::Result<()> {
        self.delegate.reportVendorAtom(atom)
    }
}
